...
impl Registry {
    ...
    /// Registers a new [`Collector`](::core::Collector) to be included in metrics
    ...
    /// Unregisters the [`Collector`](::core::Collector) that equals the [`Collector`](::core::Collector) passed
...
mod tests {
    ...
    fn test_registry() {
        ...
        let counter_vec = CounterVec::from_opts(("test_vec", "test vec help", ["a", "b"])).unwrap();
    ...
    fn test_gather_order() {
        ...
        let r = Registry::new();
        let opts = Opts::new_with_label("test", "test help", ["cc", "c1", "a2", "c0"])
            .const_label("a", "1")
            .const_label("b", "2");
        let counter_vec = CounterVec::from_opts(opts).unwrap();
        r.try_register(Box::new(counter_vec.clone())).unwrap();

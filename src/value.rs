...
impl<P: Atomic> Value<P> {
    ...
    pub fn metric(&self) -> Metric {
        ...
        m.label = from_vec!(self.label_pairs.clone());

        let val = self.get();
        match self.val_type {
            ValueType::Counter => {
                let mut counter = Counter::default();
                counter.value = val.into_f64();
                m.counter = MessageField::some(counter);
            }
            ValueType::Gauge => {
                let mut gauge = Gauge::default();
                gauge.value = val.into_f64();
                m.gauge = MessageField::some(gauge);
            }
        }
    ...
    pub fn collect(&self) -> MetricFamily {
        ...
        m.type_ = self.val_type.metric_type().into();
...
pub fn make_label_pairs(desc: &Desc, label_values: &[&str]) -> Result<Vec<LabelPair>> {
    ...
    for (i, n) in desc.variable_labels.iter().enumerate() {
        ...
        label_pair.name = n.clone();
        label_pair.value = label_values[i].to_owned();
